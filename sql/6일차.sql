...
-- ***잘 작성했는데 안될경우 CREAT ~ END 한번에 드래그 후 Ctrl + Enter***
-- ***IS는 DECLARE와는 다르게 구문을 정의할때도 쓰여서 생략이 불가능하다
-- ***IS의 역할 => 프로시저 구문 정의 + 변수선언(DECLARE)
-- ***프로시저에는 IN을 사용할 수 있지만 PL/SQL문에서는 불가능하다
-- ***프로시저에서는 DECLARE를 사용하면 오류가 난다.

-- 프로시저에서는 DECLARE 안쓰고 IS에서 선언 사용함
...
-- SELECT ... INTO
-- SQL쿼리문의 실행 결과를 PL/SQL문의 변수에 저장하는 역활
-- 쿼리문의 결과값을 직접 변수에 할당할 수 있다.
-- 결과가 없는 경우 : NO_DATA_FOUND 예외가 발생(오류)
-- 겨로가가 여러행인 경우 : TOO_MANY_ROWS 예외 발생
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('EMPLOYEES ID : '||EMP_ID);
	DBMS_OUTPUT.PUT_LINE('EMPLOYEES NAME : '||FIRST_NAME);
END;
...
-- 트리거(triger)
-- 데이터베이스에서 특정 이벤트(INSERT,UPDATE,DELETE 같은 DML또는
-- DDL 작업) 가 발생할 때 자동으로 실행하는 PL/SQL 문입니다
...
-- Statement-level 	트리거
-- DML 문장이 실행될 때 한번만 호출 된다.
-- 행의 수와 관계 없이 단 한번 실행되므로, 특정 작업 전체에 대한 처리가 필요할 때 유용하다
...
-- 감사 및 로깅
-- 테이블의 변경 내역을 별도의 감사 테이블에 기록하여, 누가 언제 어떤 데이터를 변경했는지 추적할 수 있다.

-- 비즈니스로직 구현
-- 주문 테이블에 주문이 들어오면 상품테이블에서 자동으로 재고를 수정한다.
-- 복잡한 데이터 처리
-- 한 번의 DML 작업으로 여러 관련 테이블의 동시 변경이 필요한 경우, 트리거를 
-- 이러한 복잡한 로직을 중앙집중적으로 관리할 수 잇다.
...
UPDATE EMPLOYEES e SET SALARY=-1000
WHERE EMPLOYEE_ID = 100;
...
-- BEFORE 트리거에서의 :NEW
-- :NEW의 값을 변경하여 입력될 데이터를 조정할 수 있다. //실행되기전이라
-- AFTER 트리거에서는 값이 읽기 전용이다. //실행된 후라
...
BEGIN
   IF INSERTING THEN
      ...
      VALUES (AUDIT_LOGG.NEXTVAL,'INSERT',:NEW.employee_id, '신규 직원 추가');
   ELSIF UPDATING THEN
      INSERT INTO audit_log(log_id, operation, employee_id,details)
      VALUES (AUDIT_LOGG.NEXTVAL,'UPDATE',:NEW.employee_id, '직원 정보 변경');
   ELSIF DELETING THEN
      INSERT INTO audit_log(log_id, operation, employee_id,details)
      VALUES (AUDIT_LOGG.NEXTVAL,'DELETE',:OLD.employee_id, '직원 삭제');
   END IF;
END;


CREATE SEQUENCE AUDIT_LOGG;
...
-- CACHE 20 -> 먼저  20개의 INDEX공간 확보 --잘안씀
...
INSERT INTO DEPT(DEPTNO,DNAME,LOC) VALUES (50,'개발','SEOUL');
